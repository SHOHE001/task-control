...
4. 専用カレンダーを明示的に新規作成するか、自分が所有する専用カレンダーIDを指定し確認する。メインカレンダーは拒否する。設定直後からworkerが確認済み期限と着手予定を同期するため、テスト用タスクのあるDBでは実接続しない。
...
原文は送信しません。タスクタイトル、確認済み期限、小さい最初の行動、着手予定と長さ、管理用識別子を送信します。日付のみの予定は翌日をexclusive endとする終日予定で「時刻未確認」と表示。リマインダーは既定値も含めて無効です。[Google Events insert仕様](https://developers.google.com/workspace/calendar/api/v3/reference/events/insert)
...
Google連携後、Any Planner側でtask-controlの専用カレンダーを表示対象にします。Deadlineは本人確認済み期限、Startは「課題名：小さい行動（着手）」という別イベントです。予定変更はtask-controlで行ってください。Any Plannerの内部API・Firestoreは利用しません。実機の反映時間、専用カレンダーの対応、通知・共有は本人が確認します。[設計と実機チェック](intake-design.md) を参照してください。
